...
def test_main_initializes_database(monkeypatch):
    calls = []
...
def test_event_sync_calendar_and_analytics_pipeline():
    ...
    assert sync_service.synchronize_events([first_event, second_event]) == 2
    ...
    assert len(analytics_service.find_conflicts(events)) == 1
    assert analytics_service.calculate_free_time(
        events,
        start,
        start + timedelta(hours=3),
    ) == timedelta(hours=1)
