...
class CalendarRepository:
    ...
    def find_by_account_id(self,account_id: str,) -> list:
        return self.session.query(CalendarModel).filter(CalendarModel.account_id == account_id).all()
    ...
    def find_active(self) -> list:
        return (self.session.query(CalendarModel).filter(CalendarModel.is_active.is_(True)).all())
