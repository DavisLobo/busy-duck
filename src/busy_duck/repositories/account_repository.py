...
class AccountRepository:
    ...
    def find_by_provider_id(self,provider_id: str,) -> list:
        return self.session.query(AccountModel).filter(AccountModel.provider_id == provider_id).all()

    def find_active(self,) -> list:
        return self.session.query(AccountModel).filter(AccountModel.is_active.is_(True)).all()
